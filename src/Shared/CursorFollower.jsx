...
const CursorFollower = () => {
  ...
  // Smooth follow using lerp (0 < speed < 1)
  const animate = () => {
    setCursorPos((prev) => {
      const dx = mousePos.x - prev.x;
      const dy = mousePos.y - prev.y;
      const speed = 0.15; // smaller = slower, larger = faster, must be <1
      return {
        x: prev.x + dx * speed,
        y: prev.y + dy * speed,
      };
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: clicked
          ? "0 0 15px 5px rgba(132, 204, 22, 0.7)" // glow on click
          : "0 0 20px 2px rgba(132,204,22,0.5)", // constant glow,
      }}
    />
  );
};
